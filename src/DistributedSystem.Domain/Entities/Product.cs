...
public class Product : AggregateRoot<Guid>, IAuditableEntity
{
    ...
    public static Product Create(Guid id, string name, decimal price, string description)
    {
        ...
        product.RaiseDomainEvent(new Contract.Services.V1.Product.DomainEvent.ProductCreatedDomainEvent(Guid.NewGuid(), product.Id,
    ...
    public void Update(string name, decimal price, string description)
    {
        ...
        RaiseDomainEvent(new Contract.Services.V1.Product.DomainEvent.ProductUpdatedDomainEvent(Guid.NewGuid(), Id, Name, Price, Description));
    }

    // Có instance hay đối tượng reference đến Update, ví dụ, instance.Update(...)
    public void Delete()
        => RaiseDomainEvent(new Contract.Services.V1.Product.DomainEvent.ProductDeletedDomainEvent(Guid.NewGuid(), Id));
}
